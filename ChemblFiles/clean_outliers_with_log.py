...
# Log-transform STANDARD_VALUE
data['STANDARD_VALUE'] = data['STANDARD_VALUE'].replace(0, np.nan)  # Replace zeroes to avoid log issues
data['LOG_STANDARD_VALUE'] = np.log10(data['STANDARD_VALUE'])

# Save the cleaned data
data.to_csv('output/cleaned_data.csv', index=False)
print("Outliers removed and log-transformed data saved to 'output/cleaned_data.csv'")
